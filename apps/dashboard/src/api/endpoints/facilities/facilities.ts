...
const getStudentFacilities = async (
	studentId: number,
...
const unenrollFacility = async (studentId: number, sfId: number) =>
	customInstance<any>(`/v1/students/${studentId}/facilities/${sfId}`, {
		method: "DELETE",
	});
...
export const facilityKeys = {
	...
	studentList: (studentId: number) =>
		[`/v1/students/${studentId}/facilities`] as const,
};

// ─── Hooks ───────────────────────────────────────────────────────────

export const useGetFacilities = (options?: {
...
export const useCreateFacility = (
	options?: {
		mutation?: UseMutationOptions<any, Error, CreateFacilityRequest>;
	},
	qc?: QueryClient,
) =>
	useMutation(
...
export const useUpdateFacility = (
	options?: {
		mutation?: UseMutationOptions<
			any,
			Error,
			{ id: number; data: CreateFacilityRequest }
		>;
	},
	qc?: QueryClient,
) =>
	useMutation(
		{
			mutationFn: ({ id, data }: { id: number; data: CreateFacilityRequest }) =>
...
export const useDeleteFacility = (
	...
	useMutation(
		{ mutationFn: (id: number) => deleteFacility(id), ...options?.mutation },
		qc,
	);

export const useGetStudentFacilities = (
	studentId: number,
	params?: { academic_year_id?: number },
	options?: { query?: Partial<UseQueryOptions> },
) =>
	useQuery({
		queryKey: [...facilityKeys.studentList(studentId), params],
		queryFn: () => getStudentFacilities(studentId, params),
		enabled: !!studentId,
		...options?.query,
	} as any);

export const useEnrollFacility = (
	options?: {
		mutation?: UseMutationOptions<
			any,
			Error,
			{ studentId: number; data: EnrollFacilityRequest }
		>;
	},
	qc?: QueryClient,
) =>
	useMutation(
		{
			mutationFn: ({
				...
				studentId: number;
				data: EnrollFacilityRequest;
			}) => enrollFacility(studentId, data),
			...options?.mutation,
		},
		qc,
	);

export const useUnenrollFacility = (
	options?: {
		mutation?: UseMutationOptions<
			any,
			Error,
			{ studentId: number; sfId: number }
		>;
	},
	qc?: QueryClient,
) =>
	useMutation(
		{
			mutationFn: ({ studentId, sfId }: { studentId: number; sfId: number }) =>
				unenrollFacility(studentId, sfId),
			...options?.mutation,
		},
		qc,
	);
