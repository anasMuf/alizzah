...
import type { DtoUpsertEffectiveDayRequest, DtoEffectiveDayResponse } from '../../../api/model';
...
export function EffectiveDayForm({
  ...
  const createMutation = usePostV1ClassGroupsIdEffectiveDays({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Hari efektif berhasil disimpan.' });
        queryClient.invalidateQueries({ queryKey: getGetV1ClassGroupsIdEffectiveDaysQueryKey(classGroupId) });
        onClose();
      },
      onError: (error: Error) => {
  ...
  const updateMutation = usePutV1ClassGroupsIdEffectiveDaysEdId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Hari efektif berhasil diperbarui.' });
        queryClient.invalidateQueries({ queryKey: getGetV1ClassGroupsIdEffectiveDaysQueryKey(classGroupId) });
        onClose();
      },
      onError: (error: Error) => {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <FormField
          ...
          onChange={(e) => setFormData({ ...formData, total_mondays: parseInt(e.target.value) || 0 })}
          required
        />
          ...
          <div className="flex">
            <div className="ml-3">
              ...
              <div className="mt-2 text-sm text-indigo-700">
                <p>Hari efektif digunakan untuk kalkulasi proporsi uang catering dan ekskul pada SPP/SPD.</p>
              </div>
            </div>
          </div>
        </div>
