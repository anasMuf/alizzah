...
export function BayarForm({ purchase, isOpen, onClose }: BayarFormProps) {
	const { addToast } = useToast();
	const [amount, setAmount] = useState(purchase.remaining);
	...
	}, [isOpen, purchase.remaining]);

	const payM = usePayPurchase(purchase.id);

	const handleSubmit = (e: React.FormEvent) => {
		e.preventDefault();
		if (amount <= 0 || amount > purchase.remaining) {
			addToast({
				variant: "error",
				title: "Gagal",
				message: `Nominal harus antara 1 dan ${formatCurrency(purchase.remaining)}.`,
			});
			return;
		}
		payM.mutate(
		...
		>
			<form onSubmit={handleSubmit} className="space-y-6">
				<p className="rounded-md bg-gray-50 px-3 py-2 text-sm text-gray-700">
					Sisa hutang: <strong>{formatCurrency(purchase.remaining)}</strong>
				</p>
				<CurrencyFormField
					id="amount"
