...
import { usePostV1DaycareEnrollments, getGetV1DaycareEnrollmentsQueryKey } from '../../../../api/endpoints/daycare-enrollments/daycare-enrollments';
...
export const Route = createFileRoute('/_authenticated/administrasi/daycare/baru')({
...
function DaycareBaruPage() {
  ...
  const createMutation = usePostV1DaycareEnrollments({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Siswa berhasil didaftarkan ke layanan daycare.' });
        queryClient.invalidateQueries({ queryKey: getGetV1DaycareEnrollmentsQueryKey() });
        navigate({ to: '/administrasi/daycare' });
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal mendaftarkan siswa.';
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      ...
      <nav className="flex" aria-label="Breadcrumb">
        <ol role="list" className="flex items-center space-x-2">
          <li><Link to="/administrasi/daycare" className="text-gray-400 hover:text-gray-500">Administrasi</Link></li>
          <li>
            <div className="flex items-center">
              <ChevronRight className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
              <Link to="/administrasi/daycare" className="ml-2 text-sm font-medium text-gray-500 hover:text-gray-700">Daycare</Link>
            </div>
          </li>
          <li>
            <div className="flex items-center">
              <ChevronRight className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">Pendaftaran</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                          ...
                          >
                            <div className="flex items-center">
                              <span className="ml-3 block truncate font-medium text-gray-900">
                                {student.full_name} <span className="text-gray-500 font-normal">({student.is_daycare_only ? 'Siswa Daycare Saja' : 'Siswa Reguler'})</span>
                              </span>
                            </div>
                          </div>
                        ))
                ...
                <div className="flex items-center justify-between p-4 border border-indigo-200 bg-indigo-50 rounded-lg">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-xs text-gray-500">{selectedStudent.is_daycare_only ? 'Siswa Khusus Daycare' : 'Siswa Reguler'}</p>
                    </div>
                  </div>
            ...
            <div className="sm:col-span-3">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Paket Daycare</label>
              <select
              ...
              >
                ...
                <option value="monthly_package_kb">Paket Penitipan (Khusus Daycare Usia KB)</option>
                <option value="monthly_package_tk">Paket Penitipan (Khusus Daycare Usia TK)</option>
        ...
        <div className="flex items-center justify-end gap-x-6">
          ...
          <Button type="submit" variant="primary" disabled={!selectedStudent || createMutation.isPending || !activeAy}>
