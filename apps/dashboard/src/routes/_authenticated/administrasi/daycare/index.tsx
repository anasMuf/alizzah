...
import { Plus, Search, UserCircle, ShieldX } from 'lucide-react';
import { 
...
import { getGetV1DaycareEnrollmentsQueryKey } from '../../../../api/endpoints/daycare-enrollments/daycare-enrollments';
...
function DaycareIndexPage() {
  ...
  const updateStatusMutation = usePatchV1DaycareEnrollmentsIdStatus({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Status daycare berhasil diperbarui.' });
        queryClient.invalidateQueries({ queryKey: getGetV1DaycareEnrollmentsQueryKey() });
        setIsDeactivateOpen(false);
        setEnrollmentToDeactivate(null);
      },
      onError: (error: any) => {
        addToast({ variant: 'error', title: 'Gagal', message: error?.message || 'Gagal memperbarui status.' });
      }
    }
  });
  ...
  const getPackageBadge = (pkg: string) => {
    switch (pkg) {
      case 'monthly_kb': return <Badge className="bg-blue-100 text-blue-700">Bulanan KB</Badge>;
      case 'monthly_tk': return <Badge className="bg-emerald-100 text-emerald-700">Bulanan TK</Badge>;
      case 'monthly_package_kb': return <Badge className="bg-purple-100 text-purple-700">Paket KB</Badge>;
      case 'monthly_package_tk': return <Badge className="bg-indigo-100 text-indigo-700">Paket TK</Badge>;
      case 'daily': return <Badge className="bg-amber-100 text-amber-700">Harian</Badge>;
  ...
  return (
    <div className="space-y-6">
      ...
      {!activeAy ? (
        ...
        <div className="bg-red-50 p-4 rounded-md text-red-800">Gagal memuat data daycare.</div>
            ...
            <table className="min-w-full divide-y divide-gray-300">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Paket</th>
                  <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Tanggal Mulai</th>
                  ...
                  <tr key={enrollment.id} className="hover:bg-gray-50 transition-colors">
                    <td className="whitespace-nowrap py-4 pl-4 pr-3 sm:pl-6">
                      <div className="flex items-center">
                        ...
                        <div className="ml-4">
                          ...
                          <div className="text-gray-500 text-xs">NISN: {enrollment.student.nisn || '-'}</div>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                      <div className="flex justify-end gap-2">
                        ...
                        {enrollment.status === 'active' && (
                          <button
                            onClick={() => { setEnrollmentToDeactivate(enrollment); setIsDeactivateOpen(true); }}
      ...
      <ConfirmDialog
        ...
        variant="danger"
      >
        <p>Anda yakin ingin menonaktifkan layanan daycare untuk siswa <strong>{enrollmentToDeactivate?.student?.full_name}</strong>? Status akan diubah menjadi tidak aktif mulai hari ini.</p>
      </ConfirmDialog>
    </div>
  );
}
