...
function EkskulPage() {
  ...
  const [selectedEkskul, setSelectedEkskul] = useState<DtoExtracurricularResponse | null>(null);

  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [ekskulToDelete, setEkskulToDelete] = useState<DtoExtracurricularResponse | null>(null);
  ...
  const deleteMutation = useDeleteV1ExtracurricularsId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Ekstrakurikuler berhasil dihapus.' });
        queryClient.invalidateQueries({ queryKey: getGetV1ExtracurricularsQueryKey() });
        setIsDeleteOpen(false);
        setEkskulToDelete(null);
      },
      onError: () => {
        addToast({ variant: 'error', title: 'Gagal', message: 'Tidak dapat menghapus ekstrakurikuler. Mungkin sudah ada siswa yang terdaftar.' });
        setIsDeleteOpen(false);
      }
    }
  });
  ...
  return (
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Kelola data kegiatan tambahan siswa seperti PASTA, CALISAN, dan Ekskul Reguler.
          </p>
        </div>
        <div className="mt-4 sm:ml-4 sm:mt-0">
          <Button 
            onClick={() => { setSelectedEkskul(null); setIsFormOpen(true); }}
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 p-4 rounded-md text-red-800">Gagal memuat data ekstrakurikuler.</div>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredEkskuls.map((ekskul: any) => (
            <div key={ekskul.id} className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden flex flex-col">
              <div className="p-5 flex-1">
                <div className="flex justify-between items-start mb-2">
                  {getEkskulTypeBadge(ekskul.type)}
                  <div className="flex gap-1">
                    <button onClick={() => handleEdit(ekskul)} className="p-1 text-gray-400 hover:text-indigo-600 rounded">
                      <Edit className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleDelete(ekskul)} className="p-1 text-gray-400 hover:text-red-600 rounded">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  </div>
                </div>
                <h3 className="text-lg font-bold text-gray-900 mt-2">{ekskul.name}</h3>
                <p className="mt-1 text-sm text-gray-500 line-clamp-2">Kegiatan Ekstrakurikuler</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => ekskulToDelete && deleteMutation.mutate({ id: ekskulToDelete.id as number })}
        title="Hapus Ekstrakurikuler"
        variant="danger"
      >
        <p>Anda yakin ingin menghapus <strong>{ekskulToDelete?.name}</strong>? Data ini tidak dapat dikembalikan.</p>
      </ConfirmDialog>
    </div>
  );
}
