...
import { Users, ArrowRight, ChevronRight, UserCircle } from 'lucide-react';
import { 
...
export const Route = createFileRoute('/_authenticated/administrasi/rombel/$id/')({
...
function RombelDetailPage() {
  ...
  const { data: cgResponse, isLoading: isLoadingCg } = useGetV1ClassGroupsId(classGroupId);
  const classGroup = (cgResponse as any)?.data?.data;

  // Fetch Students
  const { data: studentsResponse, isLoading: isLoadingStudents } = useGetV1ClassGroupsIdStudents(classGroupId);
  ...
  const deleteMutation = useDeleteV1ClassGroupsId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Rombel berhasil dihapus.' });
        queryClient.invalidateQueries({ queryKey: getGetV1ClassGroupsQueryKey() });
  ...
  if (!classGroup) {
    return <div className="p-8 bg-red-50 text-red-800 rounded-xl">Data rombel tidak ditemukan.</div>;
  }
    ...
    <div className="space-y-6 max-w-6xl mx-auto">
      ...
      <nav className="flex" aria-label="Breadcrumb">
        <ol role="list" className="flex items-center space-x-2">
          <li><Link to="/administrasi/rombel" className="text-gray-400 hover:text-gray-500">Administrasi</Link></li>
          <li>
            <div className="flex items-center">
              <ChevronRight className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
              <Link to="/administrasi/rombel" className="ml-2 text-sm font-medium text-gray-500 hover:text-gray-700">Rombel</Link>
            </div>
          </li>
          <li>
            <div className="flex items-center">
              <ChevronRight className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">{classGroup.name}</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div className="sm:flex sm:items-center sm:justify-between border-b border-gray-200 pb-5">
        ...
        <div className="mt-4 sm:ml-4 sm:mt-0 flex gap-3">
          <Button variant="secondary" className="flex items-center gap-2" onClick={() => setIsEditOpen(true)}>Edit Rombel</Button>
          <Button variant="danger" onClick={() => setIsDeleteConfirmOpen(true)}>Hapus Rombel</Button>
        </div>
      </div>
        ...
        <div className="space-y-6 lg:col-span-1">
          <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden">
            <div className="px-4 py-5 sm:p-6">
              <h3 className="text-base font-semibold leading-6 text-gray-900 mb-4 border-b pb-2">INFO ROMBEL</h3>
                ...
                <div className="pt-4 border-t border-gray-100">
                  ...
                  {classGroup.schedule.weekdays?.days?.length > 0 && (
                    <dd className="mb-3 bg-gray-50 p-2 rounded">
                      <p className="font-medium text-gray-700">{classGroup.schedule.weekdays.days.join(', ')}</p>
                      <p className="text-gray-500">{classGroup.schedule.weekdays.time_in} &ndash; {classGroup.schedule.weekdays.time_out}</p>
                      {classGroup.schedule.weekdays.time_out_calisan && (
                        <p className="text-xs text-indigo-600 mt-1">
                          Jika Calisan: {classGroup.schedule.weekdays.time_out_calisan}
                        </p>
                      )}
                    </dd>
                  )}
                  ...
                  {classGroup.schedule.weekend?.days?.length > 0 && (
                    <dd className="bg-gray-50 p-2 rounded">
                      <p className="font-medium text-gray-700">{classGroup.schedule.weekend.days.join(', ')}</p>
                      <p className="text-gray-500">{classGroup.schedule.weekend.time_in} &ndash; {classGroup.schedule.weekend.time_out}</p>
                    </dd>
                  )}
                </div>
              </dl>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 h-full">
            <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
              <h3 className="text-base font-semibold leading-6 text-gray-900">DAFTAR SISWA</h3>
              <p className="mt-1 text-sm text-gray-500">Total: {classGroup.student_count} siswa aktif di rombel ini.</p>
            </div>
              ...
              {isLoadingStudents ? (
                ...
                <ul role="list" className="divide-y divide-gray-100">
                  {students.map((student: any) => (
                    <li key={student.id} className="flex items-center justify-between gap-x-6 px-4 py-4 sm:px-6 hover:bg-gray-50">
                      <div className="flex min-w-0 gap-x-4">
                        {student.photo_url ? (
                          <img className="h-10 w-10 flex-none rounded-full bg-gray-50 object-cover" src={student.photo_url} alt="" />
                        ) : (
                      ...
                      >
                        Lihat Profil <span aria-hidden="true" className="ml-1">&rarr;</span>
                      </Link>
                    </li>
                  ))}
                </ul>
      ...
      <ConfirmDialog
        ...
        variant="danger"
      >
        <p>Anda yakin ingin menghapus rombel ini? Semua data yang terkait akan ikut terhapus dan tidak dapat dikembalikan.</p>
      </ConfirmDialog>
    </div>
  );
}
