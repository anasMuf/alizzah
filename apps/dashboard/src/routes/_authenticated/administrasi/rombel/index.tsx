...
function RombelIndexPage() {
  ...
  const { data: response, isLoading, isError } = useGetV1ClassGroups(
  ...
  const groupedClassGroups = classGroups.reduce((acc: any, cg: any) => {
    if (!acc[cg.level]) {
      acc[cg.level] = [];
    }
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Kelola rombongan belajar untuk tahun ajaran {activeAy?.name || 'terpilih'}.
          </p>
        </div>
                        ...
                        <div className="flex items-start text-sm text-gray-500">
                          ...
                          <div className="flex-1 space-y-1">
                            {cg.schedule.weekdays?.days?.length > 0 && (
                              <p className="line-clamp-1" title={cg.schedule.weekdays.days.join(', ')}>
                                {cg.schedule.weekdays.days.join(', ')}
                              </p>
                            )}
                            {cg.schedule.weekend?.days?.length > 0 && (
                              <p className="line-clamp-1" title={cg.schedule.weekend.days.join(', ')}>
