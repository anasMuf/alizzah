...
export const Route = createFileRoute('/_authenticated/administrasi/siklus/keluar')({
...
function SiswaKeluarPage() {
  ...
  const createMutation = usePostV1AcademicEventsWithdrawals({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: `Siswa berhasil dinonaktifkan.` });
        setIsConfirmOpen(false);
        navigate({ to: '/administrasi/siklus' });
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memproses siswa keluar.';
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
        ...
        <ol role="list" className="flex items-center space-x-2">
          ...
          <li>
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">Siswa Keluar</span>
            </div>
          </li>
        </ol>
      </nav>
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
            ...
            <div className="sm:col-span-6">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Siswa *</label>
              {!selectedStudent ? (
                      ...
                      {isSearchLoading ? (
                        ...
                        searchResults.map((student: any) => (
                          <div
                            ...
                            onClick={() => {
                              if (student.active_enrollment) setSelectedStudent(student)
                            }}
                          >
                            <div className="flex items-center justify-between">
                              ...
                              {student.active_enrollment ? (
                                ...
                                <span className="text-xs text-red-500">Tidak ada enrollment aktif</span>
                              )}
                            </div>
                          </div>
                        ))
                ...
                <div className="flex items-center justify-between p-4 border border-indigo-200 bg-indigo-50 rounded-lg">
                  <div className="flex items-center">
                    {selectedStudent.photo_url ? (
                      <img src={selectedStudent.photo_url} alt="" className="h-10 w-10 rounded-full" />
                    ) : (
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-xs text-gray-500">
                        {selectedStudent.active_enrollment?.class_group?.name} &middot; {selectedStudent.active_enrollment?.academic_year?.name}
                      </p>
                    </div>
                  </div>
                  <Button variant="ghost" size="sm" onClick={() => setSelectedStudent(null)} className="text-gray-500 hover:text-gray-700">
                    Ganti Siswa
                  </Button>
                </div>
              )}
            ...
            {selectedStudent && (
              <>
                <div className="sm:col-span-6">
                  <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Jenis Keluar *</label>
                ...
                <div className="sm:col-span-6">
                  <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Catatan</label>
                  <textarea
                    rows={3}
                  ...
                  <div className="sm:col-span-6 mt-4">
                    <div className="rounded-md bg-amber-50 p-4 border border-amber-200">
                      <div className="flex">
                        ...
                        <div className="ml-3">
                          <h3 className="text-sm font-medium text-amber-800">Perhatian Keuangan</h3>
                          <div className="mt-2 text-sm text-amber-700 space-y-2">
                            <p>
                              Tagihan aktif yang belum lunas akan dibekukan (tidak akan di-generate tagihan baru).
                              Riwayat keuangan siswa tetap dapat diakses di modul Keuangan.
                            </p>
                            <ul className="list-disc pl-5 space-y-1">
                              <li>
                                <span className="font-semibold">Tunggakan saat ini:</span>{' '}
                                {formatRupiah(financialSummary.total_unpaid)}
                              </li>
                              <li>
                                <span className="font-semibold">Saldo Tabungan Umum:</span>{' '}
                                {formatRupiah(financialSummary.savings_general_balance)}
                              </li>
                              <li>
                                <span className="font-semibold">Saldo Tabungan Wajib:</span>{' '}
                                {formatRupiah(financialSummary.savings_mandatory_balance)}
                              </li>
                            </ul>
                          </div>
                        </div>
      ...
      >
        <div className="space-y-4">
          <p>
            Anda yakin ingin menonaktifkan siswa <strong>{selectedStudent?.full_name}</strong>?
          </p>
          <div className="bg-red-50 p-3 rounded-md text-sm text-red-800 border border-red-200">
            ...
            <ul className="list-disc pl-5 space-y-1">
              <li>Status siswa akan menjadi <strong>Keluar/Pindah</strong></li>
              <li>Siswa dikeluarkan dari rombel aktifnya</li>
              <li>Sistem berhenti men-generate tagihan bulanan baru</li>
              <li>Tagihan yang belum lunas (tunggakan) dibekukan dan tetap menjadi piutang</li>
            </ul>
          </div>
          <p className="text-sm text-gray-500">Tindakan ini tidak dapat dibatalkan dari menu ini.</p>
        </div>
      </ConfirmDialog>
    </div>
  );
