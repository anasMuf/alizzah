...
export const Route = createFileRoute('/_authenticated/administrasi/siklus/kelulusan')({
...
function KelulusanPage() {
  ...
  const { data: studentsResponse, isLoading: isLoadingStudents } = useGetV1Students(
    { academic_year_id: Number(academicYearId), status: 'active', limit: 1000 },
  ...
  const createMutation = usePostV1AcademicEventsGraduations({
    mutation: {
      ...
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memproses kelulusan.';
  ...
  const handleOpenConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    if (!academicYearId) {
      addToast({ variant: 'error', title: 'Validasi Gagal', message: 'Tahun ajaran tidak valid.' });
      return;
    }
    if (graduatableStudents.length === 0) {
      addToast({ variant: 'error', title: 'Kosong', message: 'Tidak ada siswa yang bisa diproses (Rombel Berlian).' });
      return;
    }
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
        ...
        <ol role="list" className="flex items-center space-x-2">
          ...
          <li>
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">Kelulusan</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <form onSubmit={handleOpenConfirm} className="space-y-8">
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
            <div className="sm:col-span-3">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Tahun Ajaran Kelulusan *</label>
              <select
            ...
            <div className="sm:col-span-6">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Catatan Kelulusan Massal</label>
              <input
                type="text"
          ...
          <div className="flex">
            ...
            <div className="ml-3 flex-1 md:flex md:justify-between">
              <p className="text-sm text-blue-700">
                Sistem akan secara otomatis meluluskan seluruh siswa tingkat <strong>Berlian</strong>.
                Beri tanda centang pada siswa yang <strong>Tinggal Kelas</strong> (mengulang di tingkat Berlian).
                <br /><br />
                <span className="font-semibold text-red-600">Peringatan: </span> Proses ini mungkin memerlukan implementasi Backend Batch 5 untuk integrasi finansial.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl overflow-hidden">
          ...
          <div className="p-0">
            {isLoadingStudents ? (
              ...
              <div className="p-8 text-center text-gray-500">Tidak ada siswa tingkat Berlian pada tahun ajaran ini.</div>
            ) : (
        ...
        <div className="flex items-center justify-end gap-x-6 pb-12">
          ...
          <Button 
            type="submit" 
            variant="primary" 
            disabled={!academicYearId || graduatableStudents.length === 0 || createMutation.isPending}
          >
            {createMutation.isPending ? 'Memproses...' : 'Proses Kelulusan Massal'}
          </Button>
        </div>
      </form>
      ...
      >
        <div className="space-y-4">
          <p>
            Anda akan menjalankan proses kelulusan untuk <strong>{graduatableStudents.length} siswa</strong>.
          </p>
          <div className="bg-amber-50 p-3 rounded-md text-sm text-amber-800 border border-amber-200">
            ...
            <ul className="list-disc pl-5 space-y-1">
              <li>
                <strong>{graduatableStudents.length - Object.values(retainedStudents).filter(Boolean).length} Siswa</strong> akan diluluskan. Status mereka akan diubah menjadi "alumni".
              </li>
              <li>
                <strong>{Object.values(retainedStudents).filter(Boolean).length} Siswa</strong> akan mengulang di tingkat Berlian pada tahun ajaran berikutnya.
              </li>
            </ul>
          </div>
          <p className="text-sm text-gray-500 font-medium">Proses ini akan menutup seluruh pendaftaran siswa yang lulus. Apakah Anda yakin?</p>
        </div>
      </ConfirmDialog>
    </div>
  );
