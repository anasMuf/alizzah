...
export const Route = createFileRoute('/_authenticated/administrasi/siklus/kenaikan-kelas')({
...
function KenaikanKelasPage() {
  ...
  const [retainedStudents, setRetainedStudents] = useState<Record<number, boolean>>({});
  ...
  const { data: studentsResponse, isLoading: isLoadingStudents } = useGetV1Students(
    { academic_year_id: Number(fromAcademicYearId), status: 'active', limit: 1000 },
  ...
  const promotableStudents = useMemo(() => {
    return allStudents.filter((student: any) => {
      const level = student.active_enrollment?.class_group?.level?.toLowerCase();
  ...
  const createMutation = usePostV1AcademicEventsPromotions({
    mutation: {
      ...
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memproses kenaikan kelas.';
  ...
  const handleOpenConfirm = (e: React.FormEvent) => {
    e.preventDefault();
    if (!fromAcademicYearId || !toAcademicYearId || fromAcademicYearId === toAcademicYearId) {
      addToast({ variant: 'error', title: 'Validasi Gagal', message: 'Tahun ajaran asal dan tujuan tidak valid.' });
      return;
    }
    if (promotableStudents.length === 0) {
      addToast({ variant: 'error', title: 'Kosong', message: 'Tidak ada siswa yang bisa diproses.' });
      return;
    }
  ...
  const groupedStudents = useMemo(() => {
    ...
    promotableStudents.forEach((student: any) => {
      const cgName = student.active_enrollment?.class_group?.name || 'Tanpa Rombel';
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <nav className="flex" aria-label="Breadcrumb">
        <ol role="list" className="flex items-center space-x-2">
          <li><Link to="/administrasi/siklus" className="text-gray-400 hover:text-gray-500">Siklus Akademik</Link></li>
          <li>
            <div className="flex items-center">
              <ChevronRight className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">Kenaikan Kelas</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div className="border-b border-gray-200 pb-5">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Proses pemindahan siswa ke tingkat berikutnya (Mutiara &rarr; Intan, Intan &rarr; Berlian) untuk tahun ajaran baru.
        </p>
      </div>
      ...
      <form onSubmit={handleOpenConfirm} className="space-y-8">
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6">
          <h3 className="text-base font-semibold leading-7 text-gray-900 border-b pb-4 mb-4">Pengaturan Proses</h3>
          <div className="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
            <div className="sm:col-span-3">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Tahun Ajaran Asal *</label>
              <select
                value={fromAcademicYearId}
                onChange={(e) => setFromAcademicYearId(e.target.value === '' ? '' : Number(e.target.value))}
            ...
            <div className="sm:col-span-3">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Tahun Ajaran Tujuan (Baru) *</label>
              <select
              ...
              >
                <option value="">-- Pilih Tahun Ajaran --</option>
                {academicYears.map((ay: any) => (
                  <option key={ay.id} value={ay.id} disabled={ay.id === fromAcademicYearId}>
                    {ay.name}
                  </option>
                ))}
              </select>
            ...
            <div className="sm:col-span-4">
              <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Catatan Kenaikan Massal</label>
              <input
                type="text"
          ...
          <div className="flex">
            ...
            <div className="ml-3 flex-1 md:flex md:justify-between">
              <p className="text-sm text-blue-700">
                Sistem akan secara otomatis mempromosikan siswa ke tingkat berikutnya.
                Beri tanda centang pada siswa yang <strong>Tinggal Kelas</strong> (tidak naik level).
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl overflow-hidden">
          ...
          <div className="p-0">
            {isLoadingStudents ? (
              ...
              <div className="p-8 text-center text-gray-500">Tidak ada siswa Mutiara atau Intan pada tahun ajaran asal.</div>
            ) : (
                  ...
                  <div key={groupName}>
                    ...
                    <ul className="divide-y divide-gray-100">
                      {students.map((student) => {
                        ...
                        return (
                          <li key={student.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                            <div className="flex items-center">
                              <div className="h-10 w-10 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 uppercase font-bold">
                                {student.full_name.charAt(0)}
                              </div>
                              <div className="ml-4">
                                <p className={`text-sm font-medium ${isRetained ? 'text-amber-700 line-through' : 'text-gray-900'}`}>
                                  {student.full_name}
                                </p>
                                <p className="text-xs text-gray-500">NISN: {student.national_student_id || '-'}</p>
                              </div>
                            </div>
        ...
        <div className="flex items-center justify-end gap-x-6 pb-12">
          ...
          <Button 
            type="submit" 
            variant="primary" 
            disabled={!fromAcademicYearId || !toAcademicYearId || fromAcademicYearId === toAcademicYearId || promotableStudents.length === 0 || createMutation.isPending}
          >
            {createMutation.isPending ? 'Memproses...' : 'Proses Kenaikan Massal'}
          </Button>
        </div>
      </form>
      ...
      >
        <div className="space-y-4">
          <p>
            Anda akan menjalankan proses kenaikan kelas untuk <strong>{promotableStudents.length} siswa</strong>.
          </p>
          <div className="bg-amber-50 p-3 rounded-md text-sm text-amber-800 border border-amber-200">
            ...
            <ul className="list-disc pl-5 space-y-1">
              <li>
                <strong>{promotableStudents.length - Object.values(retainedStudents).filter(Boolean).length} Siswa</strong> akan dipromosikan ke tingkat berikutnya.
              </li>
              <li>
                <strong>{Object.values(retainedStudents).filter(Boolean).length} Siswa</strong> akan mengulang di tingkat yang sama.
              </li>
              <li>Siswa akan ditempatkan pada rombel default tingkat barunya. Admin wajib melakukan penyesuaian kelas via Pindah Rombel nantinya.</li>
            </ul>
          </div>
          <p className="text-sm text-gray-500 font-medium">Proses ini akan menutup seluruh enrollment aktif pada tahun ajaran asal. Apakah Anda yakin?</p>
        </div>
      </ConfirmDialog>
    </div>
  );
