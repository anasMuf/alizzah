...
export const Route = createFileRoute('/_authenticated/administrasi/siklus/mutasi')({
...
function MutasiMasukPage() {
  ...
  const createMutation = usePostV1AcademicEventsTransfers({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: `Mutasi masuk siswa berhasil diproses.` });
        navigate({ to: '/administrasi/siklus' });
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memproses mutasi masuk.';
        ...
        <ol role="list" className="flex items-center space-x-2">
          ...
          <li>
            <div className="flex items-center">
              ...
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">Mutasi Masuk</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl p-6 sm:p-8">
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                ...
                <div className="flex items-center justify-between p-4 border border-indigo-200 bg-indigo-50 rounded-lg">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-xs text-gray-500">NISN: {selectedStudent.national_student_id || '-'}</p>
                    </div>
                  </div>
            ...
            {selectedStudent && (
              <>
                <div className="sm:col-span-6">
                  <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Rombel Tujuan (Mutasi Masuk) *</label>
                  <select
                    value={formData.to_class_group_id}
                    onChange={(e) => setFormData({ ...formData, to_class_group_id: e.target.value })}
                    ...
                    required
                  >
                    <option value="">-- Pilih Rombel Intan 1 / Intan 8 --</option>
                ...
                <div className="sm:col-span-6">
                  <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Catatan Mutasi</label>
                  <textarea
                    rows={3}
                    ...
                    <div className="flex">
                      ...
                      <div className="ml-3 flex-1 md:flex md:justify-between">
                        <p className="text-sm text-blue-700">
                          Sistem akan men-generate tagihan bulanan (SPP, dll) secara proporsional sesuai dengan Tanggal Mulai Belajar untuk tahun ajaran aktif.
                        </p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="flex items-center justify-end gap-x-6">
          ...
          <Button 
            type="submit" 
            variant="primary" 
            disabled={!selectedStudent || !formData.to_class_group_id || createMutation.isPending}
          >
