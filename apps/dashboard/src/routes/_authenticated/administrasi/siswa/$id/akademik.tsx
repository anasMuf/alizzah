...
export const Route = createFileRoute('/_authenticated/administrasi/siswa/$id/akademik')({
...
function SiswaAkademikPage() {
  ...
  const { data: response, isLoading, isError } = useGetV1StudentsIdEnrollments(studentId);
  ...
  if (isError) {
    return <div className="p-8 bg-red-50 text-red-800 rounded-xl">Gagal memuat data akademik.</div>;
  }

  const getStatusBadge = (status: string) => {
    switch (status.toLowerCase()) {
      case 'active': return <Badge variant="success">Aktif</Badge>;
      case 'completed': return <Badge variant="primary">Selesai</Badge>;
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden">
      <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
        <h3 className="text-base font-semibold leading-6 text-gray-900">Riwayat Rombongan Belajar</h3>
        <p className="mt-1 max-w-2xl text-sm text-gray-500">Menampilkan daftar rombel yang pernah diikuti oleh siswa ini.</p>
      </div>
        ...
        {enrollments.length === 0 ? (
          ...
          <ul role="list" className="divide-y divide-gray-100">
            {enrollments.map((enrollment: any) => (
              <li key={enrollment.id} className="px-4 py-5 sm:px-6 hover:bg-gray-50 transition-colors">
