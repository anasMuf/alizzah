...
import { usePostV1Students, getGetV1StudentsQueryKey } from '../../../../api/endpoints/students/students';
import type { DtoCreateStudentRequest, DtoCreateGuardianInline } from '../../../../api/model';
...
function SiswaBaruPage() {
  ...
  const [guardians, setGuardians] = useState<DtoCreateGuardianInline[]>([
    { full_name: '', relationship: 'ayah', phone: '', address: '', is_primary: true }
  ]);

  const createMutation = usePostV1Students({
    mutation: {
      onSuccess: (res) => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Siswa berhasil ditambahkan.' });
        queryClient.invalidateQueries({ queryKey: getGetV1StudentsQueryKey() });
        navigate({ to: '/administrasi/siswa/$id/profil', params: { id: ((res as any).data.data.id).toString() } });
      },
      onError: (error: Error) => {
  ...
  const addGuardian = () => {
    setGuardians(prev => [
      ...prev,
      { full_name: '', relationship: 'ibu', phone: '', address: '', is_primary: prev.length === 0 }
    ]);
  };
    ...
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      ...
      <nav className="flex" aria-label="Breadcrumb">
        <ol role="list" className="flex items-center space-x-2">
          ...
          <li>
            <div className="flex items-center">
              <ChevronRight className="h-5 w-5 flex-shrink-0 text-gray-400" aria-hidden="true" />
              <span className="ml-2 text-sm font-medium text-gray-900" aria-current="page">Buat Baru</span>
            </div>
          </li>
        </ol>
      </nav>
      ...
      <div className="border-b border-gray-200 pb-5">
        ...
        <p className="mt-1 text-sm text-gray-500">
          Masukkan data siswa dan wali murid. Anda bisa menambahkan lebih dari satu wali murid.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
          <div className="px-4 py-6 sm:p-8">
            <h3 className="text-base font-semibold leading-7 text-gray-900 mb-6 border-b pb-2">DATA SISWA</h3>
            ...
            <div className="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
              ...
              <div className="sm:col-span-3">
                <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Jenis Kelamin</label>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="gender" value="L" checked={studentData.gender === 'L'} onChange={handleStudentChange} className="h-4 w-4 text-indigo-600" />
                    <span>Laki-laki</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="radio" name="gender" value="P" checked={studentData.gender === 'P'} onChange={handleStudentChange} className="h-4 w-4 text-indigo-600" />
                    <span>Perempuan</span>
                  </label>
                </div>
              </div>
              ...
              <div className="sm:col-span-3">
                <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Agama</label>
                <select
                  name="religion"
              ...
              <div className="sm:col-span-6 mt-4">
                <label className="flex items-start gap-3 cursor-pointer p-4 border rounded-lg bg-gray-50 hover:bg-gray-100 transition-colors">
                  ...
                  <div>
                    <span className="font-medium text-gray-900 block">Siswa Daycare Saja (Luar Sekolah)</span>
                    <span className="text-sm text-gray-500">Centang jika siswa ini bukan siswa reguler dan hanya dititipkan untuk layanan daycare.</span>
                  </div>
                </label>
              </div>
            </div>
        ...
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
          <div className="px-4 py-6 sm:p-8">
            <div className="flex justify-between items-center mb-6 border-b pb-2">
              <h3 className="text-base font-semibold leading-7 text-gray-900">DATA WALI MURID</h3>
              <Button type="button" variant="secondary" size="sm" onClick={addGuardian} className="flex items-center gap-2">
            ...
            <div className="space-y-8">
              {guardians.map((guardian, index) => (
                <div key={index} className="relative bg-gray-50 p-6 rounded-lg border border-gray-200">
                  <div className="absolute top-4 right-4 flex items-center gap-4">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        onChange={() => handleGuardianChange(index, 'is_primary', true)}
                        className="h-4 w-4 text-indigo-600 focus:ring-indigo-600"
                      />
                      <span className="text-sm font-medium text-indigo-900">Kontak Utama</span>
                    </label>
                    {guardians.length > 1 && (
                      <button
                        type="button"
                  ...
                  <h4 className="text-sm font-semibold text-gray-900 mb-4">Wali {index + 1}</h4>
                  ...
                  <div className="grid grid-cols-1 gap-x-6 gap-y-6 sm:grid-cols-6">
                    ...
                    <div className="sm:col-span-3">
                      <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Hubungan</label>
                      <select
                    ...
                    <div className="sm:col-span-6">
                      <label className="block text-sm font-medium leading-6 text-gray-900 mb-2">Alamat (Opsional)</label>
                      <textarea
                        rows={2}
