...
function SiswaIndexPage() {
	...
	const toggleAll = () => {
		...
		setSelected(
			new Set(
				students
					.filter((s: any) => !s.active_enrollment && s.status === "active")
					.map((s: any) => s.id),
			),
		);
	};
	...
	const batchMutation = useMutation({
		mutationFn: async (data: {
			...
			const resp = await customInstance<any>("/v1/students/enrollments/batch", {
				method: "POST",
				...
				},
				{
					onSuccess: (res: any) => {
						...
						addToast({
							variant: "success",
							title: "Berhasil",
							message: `${data?.success || ids.length} siswa berhasil didaftarkan.`,
						});
	...
	return (
		<div className="space-y-6">
			...
			{selected.size > 0 && (
				<div className="bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3 flex items-center justify-between">
					...
					<div className="flex gap-2">
						...
						<Button size="sm" onClick={openBulkEnroll}>
							<GraduationCap className="h-4 w-4 mr-1" /> Daftarkan ke Rombel
						</Button>
					</div>
				</div>
			)}
			...
			{/* Table */}
			{!activeAy ? (
				<EmptyState title="Menunggu Tahun Ajaran" />
			) : isLoading ? (
				<div className="bg-white rounded-xl shadow-sm h-96 animate-pulse p-8">
						...
						<table className="min-w-full divide-y divide-gray-300">
							<thead className="bg-gray-50">
								<tr>
									...
									<th className="py-3.5 pr-3 text-left text-sm font-semibold text-gray-900">
										Siswa
									</th>
												...
												<div className="flex items-center justify-end gap-2">
													{canEnroll && (
														<button
															...
															className="text-amber-600 hover:text-amber-800 bg-amber-50 px-3 py-1 rounded-md text-sm"
														>
			...
			<SlideOver
				...
				title={isBulk ? `Daftarkan ${selected.size} Siswa` : "Daftarkan Siswa"}
			>
				<form onSubmit={handleEnroll} className="space-y-4">
					{isBulk ? (
						<p className="text-sm text-gray-600">
							{selected.size} siswa akan didaftarkan ke rombel yang sama.
						</p>
					) : (
						<div>
							<p className="text-sm text-gray-600 mb-1">Siswa</p>
							<p className="font-semibold text-gray-900">
								{students.find((s: any) => s.id === enrollStudentId)?.full_name}
							</p>
						</div>
					)}
					<div>
						...
						<select
							className="block w-full rounded-md border-0 py-2 pl-3 pr-10 text-gray-900 ring-1 ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm"
							value={enrollClassGroupId}
							onChange={(e) => setEnrollClassGroupId(e.target.value)}
							required
						>
					...
					<div>
						...
						<input
							type="date"
							className="block w-full rounded-md border-0 py-2 px-3 text-gray-900 ring-1 ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm"
