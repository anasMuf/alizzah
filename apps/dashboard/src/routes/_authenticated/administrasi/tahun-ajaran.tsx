...
export const Route = createFileRoute('/_authenticated/administrasi/tahun-ajaran')({
...
function AcademicYearsPage() {
  ...
  const [selectedAy, setSelectedAy] = useState<DtoAcademicYearResponse | null>(null);
  
  const [isConfirmActivateOpen, setIsConfirmActivateOpen] = useState(false);
  const [ayToActivate, setAyToActivate] = useState<DtoAcademicYearResponse | null>(null);

  const activateMutation = usePatchV1AcademicYearsIdActivate({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Tahun ajaran diaktifkan.' });
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <EmptyState 
          ...
          action={<Button onClick={handleCreate}>Buat Tahun Ajaran Baru</Button>}
        />
      ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex flex-col">
                    <div className="flex items-center gap-3">
                      <p className="truncate text-lg font-medium text-indigo-600">{ay.name}</p>
                      {ay.is_active && (
                    ...
                    <div className="mt-2 flex items-center text-sm text-gray-500 gap-1">
                      ...
                      <p>
                        {formatDate(ay.start_date)} &ndash; {formatDate(ay.end_date)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    {!ay.is_active && (
                      <Button variant="secondary" size="sm" onClick={() => handleActivateClick(ay)}>
                        Aktifkan
                      </Button>
                    )}
                    <Button variant="secondary" onClick={() => handleEdit(ay)}>Edit</Button>
                  </div>
                </div>
              </li>
            ))}
      ...
      <ConfirmDialog
        ...
        variant="primary"
      >
        <p>Anda yakin ingin mengaktifkan Tahun Ajaran <strong>{ayToActivate?.name}</strong>? Tindakan ini akan menonaktifkan tahun ajaran yang sedang aktif saat ini.</p>
      </ConfirmDialog>
    </div>
  );
}
