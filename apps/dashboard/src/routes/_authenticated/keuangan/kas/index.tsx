...
import { useGetV1CashBalance, getGetV1CashBalanceQueryKey } from '../../../../api/endpoints/cash/cash';
import { useGetV1CashTransactions, getGetV1CashTransactionsQueryKey } from '../../../../api/endpoints/cash/cash';
import { usePostV1CashTransfers } from '../../../../api/endpoints/cash/cash';
import { useGetV1VaultBalance, getGetV1VaultBalanceQueryKey } from '../../../../api/endpoints/vault/vault';
    ...
    transferMutation.mutate(
      ...
      },
      {
        onSuccess: () => {
          ...
          setTransferAmount('');
          setTransferNote('');
          queryClient.invalidateQueries({ queryKey: getGetV1CashBalanceQueryKey() });
          queryClient.invalidateQueries({ queryKey: getGetV1VaultBalanceQueryKey() });
          queryClient.invalidateQueries({ queryKey: getGetV1CashTransactionsQueryKey() });
        },
        onError: () => {
          addToast({
            variant: 'error',
            title: 'Transfer Gagal',
            message: 'Terjadi kesalahan saat melakukan transfer. Silakan coba lagi.',
          });
        },
      }
    );
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {[0, 1].map((i) => (
            <div key={i} className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-6">
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-sm font-medium ${isClosedToday ? 'text-green-600' : 'text-yellow-600'}`}>
                  {lastClosingDate ? formatDate(lastClosingDate) : 'Belum pernah'}
                </span>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-6">
            ...
            <div className="space-y-3 mb-6">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Tab. Umum (semua siswa)</span>
                <span className="text-sm font-medium text-gray-900">
                  {formatCurrency(savingsGeneral)}
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-600">Tab. Wajib (Berlian)</span>
                <span className="text-sm font-medium text-gray-900">
      ...
      <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-200">
          <h3 className="text-base font-semibold text-gray-900">MUTASI HARI INI</h3>
        </div>
              ...
              return (
                <div key={tx.id} className="px-6 py-3 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className={`flex h-8 w-8 items-center justify-center rounded-full ${isCredit ? 'bg-green-50' : 'bg-red-50'}`}>
                      {isCredit ? (
                        <ArrowUpRight className="h-4 w-4 text-green-500" />
                      ) : (
                        <ArrowDownRight className="h-4 w-4 text-red-500" />
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1 text-xs text-gray-500">
                        ...
                        {tx.created_by?.full_name && (
                          <span className="ml-1">&middot; {tx.created_by.full_name}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <span className={`text-sm font-semibold tabular-nums ${isCredit ? 'text-green-600' : 'text-red-600'}`}>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            {parsedAmount > cashBalance && (
              <p className="mt-1 text-sm text-red-600">
                Nominal tidak boleh melebihi saldo kas ({formatCurrency(cashBalance)})
              </p>
            )}
          </div>
          ...
          {parsedAmount > 0 && isAmountValid && (
            <div className="rounded-lg border border-gray-200 p-4 space-y-3">
              <p className="text-sm font-medium text-gray-900">Pratinjau Setelah Transfer</p>
              <div className="flex items-center justify-between">
