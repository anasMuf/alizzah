...
function KasTransaksiPage() {
  ...
  const groupedByDate = useMemo(() => {
    ...
    for (const tx of transactions) {
      const dateKey = (tx.transaction_date || tx.created_at || '').split('T')[0];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm ring-1 ring-gray-900/5 space-y-4">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          <div className="w-full sm:w-auto">
            <label className="block text-sm font-medium leading-6 text-gray-900 mb-1">Jenis</label>
            <select
              value={jenis}
          ...
          <div className="w-full sm:w-auto">
            <label className="block text-sm font-medium leading-6 text-gray-900 mb-1">Tipe</label>
            <select
              value={tipe}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-4">
          ...
          <p className="text-xl font-bold text-gray-900">{formatCurrency(totalCredit - totalDebit)}</p>
        </div>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-300">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="px-3 py-3.5 text-right text-sm font-semibold text-gray-900 pr-6">
                  Nominal
                </th>
              </tr>
            </thead>
                        ...
                        <tr key={tx.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-3 text-sm text-gray-900">
                            ...
                            {tx.created_by?.full_name && (
                              <p className="text-xs text-gray-500 mt-0.5">oleh {tx.created_by.full_name}</p>
                            )}
                          </td>
                          <td className="whitespace-nowrap px-3 py-3 text-sm text-right pr-6">
                            <span className={`inline-flex items-center gap-1 font-semibold tabular-nums ${isCredit ? 'text-green-600' : 'text-red-600'}`}>
                              {isCredit ? (
        ...
        {meta && meta.total_pages > 1 && (
          <div className="border-t border-gray-200 px-4 py-3 flex items-center justify-between sm:px-6">
            <div className="flex flex-1 justify-between sm:hidden">
              ...
              <Button variant="secondary" onClick={() => setPage((p) => p + 1)} disabled={page >= meta.total_pages}>
                Next
              </Button>
            </div>
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Menampilkan{' '}
                  <span className="font-medium">{(page - 1) * 30 + 1}</span> sampai{' '}
                  <span className="font-medium">{Math.min(page * 30, meta.total_items)}</span> dari{' '}
                  <span className="font-medium">{meta.total_items}</span> transaksi
                </p>
              </div>
              <div>
