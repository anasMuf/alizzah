...
function LaporanPengeluaranPage() {
  ...
  const { data: reportData, isLoading, isError } = useGetReportsTransaksiPengeluaran(
    {
      month: reportMonth,
      year: reportYear,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          <nav className="flex items-center text-sm text-gray-500 mb-2">
            ...
            <span className="text-gray-900 font-medium">Transaksi Pengeluaran</span>
          </nav>
          <h2 className="text-2xl font-bold leading-7 text-gray-900 sm:truncate sm:tracking-tight">
            {report
              ? `Transaksi Pengeluaran — ${MONTH_NAMES[reportMonth - 1]} ${reportYear}`
      ...
      <div className="hidden print:block border-b border-gray-300 pb-4 mb-6">
        ...
        <p className="text-sm text-gray-600 text-center">Laporan Transaksi Pengeluaran</p>
        ...
        <>
          {transactions.length > 0 ? (
              ...
              <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-6">
                <div className="flex justify-between items-center">
                  <span className="text-base font-bold text-gray-900">
                    Grand Total Pengeluaran {MONTH_NAMES[reportMonth - 1]} {reportYear}
                  </span>
            ...
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-12 text-center">
              <p className="text-sm text-gray-500">Tidak ada transaksi pengeluaran pada bulan ini.</p>
            </div>
          )}
        </>
      )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
          <div className="space-y-1">
            <div className="flex gap-2">
              <span className="text-gray-500 w-32 shrink-0">Cara Transaksi</span>
              <span className="text-gray-900 font-semibold">: {sourceLabel}</span>
            </div>
            <div className="flex gap-2">
            ...
            <div className="flex gap-2">
              ...
              <span className="text-gray-700 italic">: {txn.total_terbilang}</span>
            </div>
          </div>
          <div className="space-y-1 sm:text-right">
            <div className="flex sm:justify-end gap-2">
              <span className="text-gray-500">Tgl. Transaksi :</span>
              <span className="text-gray-900 font-medium">{formatDateID(txn.transaction_date)}</span>
            </div>
            <div className="flex sm:justify-end gap-2">
      ...
      <div className="px-6 py-3">
        ...
        <table className="min-w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            {txn.items.map((item) => (
              <tr key={item.no}>
                <td className="py-1.5 text-sm text-gray-500 tabular-nums">{item.no}.</td>
                <td className="py-1.5 text-sm text-gray-700 font-medium">{item.category_name}</td>
                <td className="py-1.5 text-sm text-gray-900">{item.description}</td>
