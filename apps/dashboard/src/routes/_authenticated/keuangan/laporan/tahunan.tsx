...
function LaporanTahunanPage() {
  ...
  const totalNet = Number(income?.total_paid || 0) - Number(expense?.total || 0);
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wider font-medium">Total Tagihan</p>
            ...
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wider font-medium">Total Pengeluaran</p>
            ...
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-5">
              ...
              <p className={`mt-1 text-xl font-bold tabular-nums ${Number(report.net || totalNet) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wider font-medium">Saldo Kas</p>
              <p className="mt-1 text-xl font-bold text-gray-900 tabular-nums">
                {formatCurrency(Number(report.cash_balance || 0))}
              </p>
            </div>
            <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-5">
              <p className="text-xs text-gray-500 uppercase tracking-wider font-medium">Saldo Berangkas</p>
              <p className="mt-1 text-xl font-bold text-gray-900 tabular-nums">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-300">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="py-3 pl-6 pr-3 text-left text-sm font-semibold text-gray-900">Bulan</th>
                    <th className="px-3 py-3 text-right text-sm font-semibold text-gray-900">Pemasukan</th>
                    <th className="px-3 py-3 text-right text-sm font-semibold text-gray-900">Pengeluaran</th>
                    <th className="px-3 py-3 text-right text-sm font-semibold text-gray-900 pr-6">Net</th>
                  </tr>
                </thead>
                  ...
                  {months.length > 0 ? (
                    <>
                      {months.map((m: any, i: number) => {
                        ...
                        return (
                          <tr key={i}>
                            ...
                            <td className={`px-3 py-3 text-sm text-right tabular-nums font-medium pr-6 ${net >= 0 ? 'text-gray-900' : 'text-red-600'}`}>
                              {formatCurrency(net)}
                            </td>
                          </tr>
                        );
                      ...
                      <tr className="bg-gray-50 font-semibold">
                        ...
                        <td className="px-3 py-3 text-sm text-right tabular-nums text-green-600">
                          {formatCurrency(months.reduce((s: number, m: any) => s + Number(m.income || 0), 0))}
                        </td>
                        <td className="px-3 py-3 text-sm text-right tabular-nums text-red-600">
                          {formatCurrency(months.reduce((s: number, m: any) => s + Number(m.expense || 0), 0))}
                        </td>
                        <td className="px-3 py-3 text-sm text-right tabular-nums font-bold pr-6 text-gray-900">
                          {formatCurrency(
                            months.reduce((s: number, m: any) => s + Number(m.income || 0) - Number(m.expense || 0), 0),
                          )}
                        </td>
                      </tr>
                    </>
