...
function KasirPembayaranPage() {
  ...
  const searchParams = useSearch({ from: '/_authenticated/keuangan/pembayaran/baru' });
  const initialStudentId = (searchParams as any).student_id ? Number((searchParams as any).student_id) : null;
  const initialInvoiceId = (searchParams as any).invoice_id ? Number((searchParams as any).invoice_id) : null;
  ...
  const { data: initialStudentResp } = useGetV1StudentsId(initialStudentId || 0, { query: { enabled: !!initialStudentId } });
  useEffect(() => {
  ...
  // Savings
  const { data: savingsResp } = useGetV1StudentsIdSavings(selectedStudent?.id || 0, { query: { enabled: !!selectedStudent?.id } });
  ...
  const { data: invoiceDetails = [] } = useGetV1InvoicesBatch(selectedInvoices, { enabled: selectedInvoices.length > 0 });
    ...
    invoiceDetails.forEach((detail: any) => {
      detail?.items?.forEach((item: any) => {
        ...
        if (sisa > 0 || item.category === 'dispensation') {
          items.push({ id: item.id, invoice_id: detail.id, name: item.name, category: item.category, sisa_tagihan: sisa, is_dispensation: item.category === 'dispensation' });
        }
      });
    });
    return items;
  ...
  // Derived
  const tabunganUmumTotal = incidentalItems.filter(i => i.isSavings).reduce((sum, i) => sum + i.amount, 0);
  ...
  const paymentMutation = usePostV1Payments({
    mutation: {
      onSuccess: (res: any) => {
        ...
        navigate({ to: '/keuangan/pembayaran/$id', params: { id: String(res.data?.data?.id) } });
      },
      onError: (err: any) => {
  ...
  const handleSubmit = () => {
    ...
    const totalSavingsDeposit = tabunganUmumTotal + (depositChange && Number(cashReceived) > totalPay ? Number(cashReceived) - totalPay : 0);
        ...
        <div className="flex-1 flex min-h-0">
          ...
          <div className="w-1/2 border-r border-gray-200 flex flex-col bg-white">
            <div className="flex-1 overflow-y-auto p-4 space-y-3">
              ...
              <InvoiceSelector
                ...
                onToggleInvoice={(id) => setSelectedInvoices(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])}
                onAmountChange={(itemId, val) => setPayAmounts(prev => ({ ...prev, [itemId]: Number(val) }))}
              />
            </div>
            <div className="flex-shrink-0 border-t border-gray-200 p-4 bg-gray-50">
              <IncidentalItems items={incidentalItems} onChange={setIncidentalItems} />
            </div>
          </div>
          ...
          <div className="w-1/2 flex flex-col bg-gray-50">
            ...
            <div className="flex-shrink-0 border-t border-gray-200 p-4 bg-white">
              <Button variant="primary" className="w-full justify-center py-3 text-base" onClick={handleSubmit} disabled={!canSubmit || paymentMutation.isPending}>
