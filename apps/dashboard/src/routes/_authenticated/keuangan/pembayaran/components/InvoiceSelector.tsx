...
export function InvoiceSelector({
  ...
  const invoiceItems = useMemo(() => {
    ...
    items.sort((a, b) => (a.is_dispensation ? 1 : 0) - (b.is_dispensation ? 1 : 0));
    return items;
  }, [invoiceDetails]);
  ...
  if (isLoading) {
    return <div className="animate-pulse space-y-2">{[1, 2, 3].map(n => <div key={n} className="h-12 bg-gray-100 rounded" />)}</div>;
  }

  if (unpaidInvoices.length === 0) {
    return <div className="bg-green-50 p-3 rounded-md text-sm text-green-700">Tidak ada tagihan tertunggak.</div>;
  }
        ...
        {unpaidInvoices.map((inv: any) => {
          ...
          return (
            <label key={inv.id} className={`flex items-center p-2.5 border rounded-lg cursor-pointer text-sm transition-colors ${isSelected ? 'border-indigo-500 bg-indigo-50' : 'border-gray-200 hover:bg-gray-50'}`}>
              <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" checked={isSelected} onChange={() => onToggleInvoice(inv.id)} />
