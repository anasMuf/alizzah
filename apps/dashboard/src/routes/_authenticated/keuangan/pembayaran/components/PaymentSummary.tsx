  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      <div className="bg-white rounded-lg border border-gray-200 divide-y divide-gray-100">
        {invoiceItems.filter((i: any) => payAmounts[i.id] !== 0).map((item: any) => (
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className={`border rounded-lg p-3 cursor-pointer flex items-center gap-2 text-sm ${source === 'cash' ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-600' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
            <input type="radio" className="sr-only" checked={source === 'cash'} onChange={() => onSourceChange('cash')} />
            <Wallet className={`w-5 h-5 ${source === 'cash' ? 'text-indigo-600' : 'text-gray-400'}`} />
            <span className="font-medium">Tunai</span>
          </label>
          <label className={`border rounded-lg p-3 cursor-pointer flex items-center gap-2 text-sm ${source === 'savings' ? 'border-indigo-600 bg-indigo-50 ring-1 ring-indigo-600' : 'border-gray-200 bg-white hover:bg-gray-50'}`}>
            <input type="radio" className="sr-only" checked={source === 'savings'} onChange={() => onSourceChange('savings')} />
            <FileText className={`w-5 h-5 ${source === 'savings' ? 'text-indigo-600' : 'text-gray-400'}`} />
            <div>
              <span className="font-medium">Tabungan</span>
              <span className="block text-xs text-gray-500">{formatCurrency(savingsBalance)}</span>
            </div>
          </label>
        </div>
      </div>
      ...
      {source === 'cash' && totalPay > 0 && (
        <div className="space-y-3">
          <div>
            ...
            <input type="number" className="block w-full rounded-lg border-0 py-2.5 px-4 text-lg font-bold text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 tabular-nums" value={cashReceived} onChange={(e) => onCashReceivedChange(e.target.value)} placeholder="0" />
          </div>
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-3 space-y-2">
              ...
              {changeAmount > 0 && (
                <label className="flex items-center pt-2 border-t border-gray-100">
                  <input type="checkbox" className="h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-600" checked={depositChange} onChange={(e) => onDepositChangeChange(e.target.checked)} />
      ...
      {totalPay > 0 && (
        <div>
          ...
          <input type="text" className="block w-full rounded-lg border-0 py-1.5 px-3 text-sm text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600" placeholder="Catatan tambahan..." value={notes} onChange={(e) => onNotesChange(e.target.value)} />
        </div>
      )}
    </div>
  );
