...
export function StudentSearch({ selectedStudent, onSelect, onClear }: StudentSearchProps) {
  ...
  return (
    <>
      ...
      <input
        ...
        autoFocus
      />
      {isLoading && <p className="absolute left-0 top-full mt-1 text-xs text-gray-500 bg-white px-3 py-1 rounded shadow">Mencari...</p>}
      {!isLoading && students.length > 0 && debouncedSearch.length > 1 && (
        <ul className="absolute z-20 left-0 top-full mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-52 overflow-y-auto">
          {students.map((s: any) => (
            <li key={s.id} className="px-4 py-2.5 hover:bg-indigo-50 cursor-pointer text-sm border-b border-gray-100 last:border-0" onClick={() => handleSelect(s)}>
