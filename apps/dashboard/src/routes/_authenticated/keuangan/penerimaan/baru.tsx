...
export const Route = createFileRoute('/_authenticated/keuangan/penerimaan/baru')({
...
function PenerimaanBaruPage() {
  ...
  const createMutation = useCreateIncomeTransaction({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Penerimaan berhasil dicatat.' });
      ...
      onError: (err: any) => {
        addToast({ variant: 'error', title: 'Gagal', message: err.message || 'Gagal mencatat penerimaan.' });
      },
    },
  });
  ...
  const canSubmit = !!sourceName.trim() && Number(amount) > 0 && !!transactionDate;
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div>
        <h2 className="text-2xl font-bold leading-7 text-gray-900">Catat Penerimaan Baru</h2>
        <p className="mt-1 text-sm text-gray-500">Catat penerimaan dana bantuan dari BOS, donasi, hibah, atau sumber lainnya.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
        <div className="px-4 py-6 sm:p-8 space-y-6">
          ...
          {/* Catatan */}
          <div>
            <label htmlFor="notes" className="block text-sm font-medium leading-6 text-gray-900 mb-2">
              Catatan
            </label>
            <textarea
              id="notes"
