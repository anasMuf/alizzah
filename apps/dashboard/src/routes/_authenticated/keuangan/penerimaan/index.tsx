...
function PenerimaanListPage() {
  ...
  const { data: listData, isLoading, isError } = useGetIncomeTransactions(
    {
      page,
      limit: 20,
  ...
  // Summary
  const totalAmount = items.reduce((sum: number, t: any) => sum + Number(t.amount), 0);
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm ring-1 ring-gray-900/5">
        <div className="flex flex-wrap gap-4 items-end">
          ...
          {(selectedCategory || dateFrom || dateTo) && (
            <button
              type="button"
              onClick={() => { setSelectedCategory(''); setDateFrom(''); setDateTo(''); setPage(1); }}
      ...
      {isLoading && (
        <div className="animate-pulse space-y-3">
          {[1, 2, 3].map((n) => <div key={n} className="h-16 bg-gray-200 rounded-lg" />)}
        </div>
      )}
      {isError && <Alert variant="error" title="Gagal Memuat">Terjadi kesalahan saat memuat data penerimaan.</Alert>}
                ...
                <table className="min-w-full divide-y divide-gray-300">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-3 py-3 text-left text-sm font-semibold text-gray-900">Kategori</th>
                      <th className="px-3 py-3 text-left text-sm font-semibold text-gray-900">Sumber / Pengirim</th>
                      <th className="px-3 py-3 text-left text-sm font-semibold text-gray-900">Ref</th>
                      <th className="px-3 py-3 text-right text-sm font-semibold text-gray-900">Nominal</th>
                  ...
                  <tfoot>
                    <tr className="bg-gray-50 border-t-2 border-gray-300">
                      <td colSpan={4} className="py-3 pl-6 text-sm font-bold text-gray-900 text-right">
                        Total Halaman Ini
                      </td>
      ...
      >
        <p>
          Apakah Anda yakin ingin menghapus penerimaan <strong>{deletingItem?.source_name}</strong> senilai{' '}
