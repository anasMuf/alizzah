...
import { ArrowLeft, Save, Trash2, ExternalLink, Edit, X } from 'lucide-react';
import {
  useGetV1ExpensesId,
  usePutV1ExpensesId,
...
export const Route = createFileRoute('/_authenticated/keuangan/pengeluaran/$id')({
...
function PengeluaranDetailPage() {
  ...
  const { data: expenseResponse, isLoading, isError } = useGetV1ExpensesId(expenseId);
  ...
  const subCategories = useMemo(() => {
    ...
    const parent = categories.find((c: any) => c.id === Number(parentCategoryId));
  ...
  const updateMutation = usePutV1ExpensesId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Pengeluaran berhasil diperbarui.' });
        queryClient.invalidateQueries({ queryKey: [`/v1/expenses/${expenseId}`] });
        queryClient.invalidateQueries({ queryKey: ['/v1/expenses'] });
        setIsEditing(false);
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memperbarui pengeluaran.';
        setFormError(msg);
      },
    },
  });
  ...
  const deleteMutation = useDeleteV1ExpensesId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Pengeluaran berhasil dihapus.' });
        navigate({ to: '/keuangan/pengeluaran' });
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal menghapus pengeluaran.';
  ...
  const handleSave = () => {
    setFormError('');
    if (!subCategoryId || !amount || Number(amount) <= 0 || !description.trim()) {
    ...
    <div className="space-y-6 max-w-3xl mx-auto">
      <div>
        ...
        <div className="sm:flex sm:items-center sm:justify-between">
          <div>
            <h1 className="text-2xl font-bold leading-7 text-gray-900">Detail Pengeluaran</h1>
            <p className="mt-1 text-sm text-gray-500">ID #{expense.id}</p>
          </div>
          <div className="mt-4 sm:mt-0 flex gap-2">
            {isEditing ? (
              <>
                <Button variant="secondary" onClick={handleCancelEdit} disabled={updateMutation.isPending}>
                  <X className="h-4 w-4 mr-1" /> Batal
                </Button>
                <Button variant="primary" onClick={handleSave} disabled={updateMutation.isPending}>
                  <Save className="h-4 w-4 mr-1" /> {updateMutation.isPending ? 'Menyimpan...' : 'Simpan'}
                </Button>
              </>
            ) : (
              <>
        ...
        <dl className="divide-y divide-gray-200">
          ...
          <Field label="Nominal">
            {isEditing ? (
              ...
              <span className="text-lg font-semibold text-gray-900">{formatCurrency(Number(expense.amount))}</span>
            )}
          </Field>
          ...
          <Field label="Dicatat oleh">
            <span className="text-gray-900">{expense.created_by?.full_name || '-'}</span>
          </Field>

          <Field label="Waktu Dicatat">
            <span className="text-gray-900">{formatDateTime(expense.created_at)}</span>
          </Field>
        </dl>
      </div>
