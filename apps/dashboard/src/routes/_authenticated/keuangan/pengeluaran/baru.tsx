...
function CatatPengeluaranPage() {
  ...
  const createMutation = usePostV1Expenses({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Pengeluaran berhasil dicatat.' });
      ...
      onError: (err: any) => {
        ...
        if (status === 422) {
          const message = data?.message || 'Tanggal ini sudah ditutup buku. Tidak dapat mencatat pengeluaran pada tanggal tersebut.';
        ...
        setFormError(data?.message || err?.message || 'Gagal menyimpan pengeluaran.');
      },
    },
  });
  ...
  const validate = (): boolean => {
    ...
    if (!subCategoryId) errors.expense_category_id = 'Sub-kategori wajib dipilih.';
    if (!amount || Number(amount) <= 0) errors.amount = 'Nominal harus lebih dari 0.';
      ...
      <form onSubmit={handleSubmit}>
        <div className="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
          <div className="px-4 py-6 sm:p-8 space-y-6">
            <div>
              <label htmlFor="expense_date" className="block text-sm font-medium leading-6 text-gray-900">
              ...
              <div className="mt-2">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setFieldErrors((prev) => { const n = { ...prev }; delete n.expense_date; return n; });
                  }}
                ...
                {fieldErrors.expense_date && (
                  <p className="mt-1 text-sm text-red-600">{fieldErrors.expense_date}</p>
                )}
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <div>
                <label htmlFor="parent_category" className="block text-sm font-medium leading-6 text-gray-900">
                ...
                <div className="mt-2">
                  ...
                  {fieldErrors.parent_category && (
                    <p className="mt-1 text-sm text-red-600">{fieldErrors.parent_category}</p>
                  )}
                </div>
              </div>

              <div>
                <label htmlFor="sub_category" className="block text-sm font-medium leading-6 text-gray-900">
                ...
                <div className="mt-2">
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      setFieldErrors((prev) => { const n = { ...prev }; delete n.expense_category_id; return n; });
                    }}
                  ...
                  >
                    <option value="">{parentCategoryId ? 'Pilih Sub-kategori' : 'Pilih kategori terlebih dahulu'}</option>
                    {subCategories.map((cat: any) => (
                      <option key={cat.id} value={cat.id}>{cat.name}</option>
                    ))}
                  </select>
                  {fieldErrors.expense_category_id && (
                    <p className="mt-1 text-sm text-red-600">{fieldErrors.expense_category_id}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div>
              <label htmlFor="receipt" className="block text-sm font-medium leading-6 text-gray-900">
                Bukti Pengeluaran <span className="text-gray-400">(opsional)</span>
              </label>
              <div className="mt-2">
                <input
                  id="receipt"
