...
export const Route = createFileRoute('/_authenticated/keuangan/pengeluaran/kategori')({
...
function ExpenseCategoryPage() {
  ...
  const [editingCategory, setEditingCategory] = useState<DtoExpenseCategoryResponse | null>(null);
  const [parentIdForNew, setParentIdForNew] = useState<number | undefined>(undefined);

  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const [categoryToDelete, setCategoryToDelete] = useState<DtoExpenseCategoryResponse | null>(null);

  const { data: response, isLoading, isError } = useGetV1ExpenseCategories();
  const categories: DtoExpenseCategoryResponse[] = (response?.data as any)?.data || [];

  const invalidate = () => queryClient.invalidateQueries({ queryKey: getGetV1ExpenseCategoriesQueryKey() });

  const deleteMutation = useDeleteV1ExpenseCategoriesId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Kategori berhasil dihapus.' });
        invalidate();
        setIsDeleteOpen(false);
        setCategoryToDelete(null);
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal menghapus kategori. Mungkin masih digunakan oleh pengeluaran.';
  ...
  return (
    <div className="space-y-6">
      <div className="sm:flex sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            Kelola kategori dan sub-kategori pengeluaran operasional (maksimal 2 level).
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 p-4 rounded-md text-red-800">Gagal memuat data kategori.</div>
      ) : categories.length === 0 ? (
        <EmptyState
        ...
        <div className="space-y-4">
          {categories.map((parent) => (
            <div key={parent.id} className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 overflow-hidden">
              ...
              {parent.children && parent.children.length > 0 ? (
                <ul className="divide-y divide-gray-100">
                  {parent.children.map((child) => (
                    <li key={child.id} className="px-5 py-3 flex items-center justify-between hover:bg-gray-50">
      ...
      <ConfirmDialog
        ...
        onConfirm={() => categoryToDelete && deleteMutation.mutate({ id: categoryToDelete.id as number })}
        title="Hapus Kategori"
        variant="danger"
        confirmLabel="Hapus"
        cancelLabel="Batal"
...
function CategoryFormSlideOver({
  ...
  const createMutation = usePostV1ExpenseCategories({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Kategori berhasil ditambahkan.' });
        onSuccess();
        onClose();
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal menambahkan kategori.';
        addToast({ variant: 'error', title: 'Gagal', message: msg });
      },
    },
  });
  ...
  const updateMutation = usePutV1ExpenseCategoriesId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Kategori berhasil diperbarui.' });
        onSuccess();
        onClose();
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memperbarui kategori.';
        addToast({ variant: 'error', title: 'Gagal', message: msg });
      },
    },
  });
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <FormField
          ...
          name="name"
          label="Nama Kategori"
          placeholder={parentId ? 'e.g. ATK, Listrik, dll.' : 'e.g. Operasional, Personalia, dll.'}
          value={name}
        ...
        {parentId && (
          <p className="text-sm text-gray-500">
            Sub-kategori ini akan ditambahkan di bawah kategori induk yang dipilih.
          </p>
        )}
      </form>
    </SlideOver>
