...
function DetailTabunganSiswaPage() {
	...
	const { data: savingsResp, isLoading: isSavingsLoading } =
		useGetV1StudentsIdSavings(Number(id), {
			query: { enabled: !!id },
		});
		...
		mutation: {
			...
			onError: (err: any) => {
				addToast({
					variant: "error",
					title: "Gagal",
					message: err.message || "Gagal memproses penarikan.",
				});
			},
		},
	});
	...
	const { data: printReportResp, isLoading: isPrintLoading } =
		useGetReportsTabunganSiswa(
			Number(id),
			{
	...
	const printReport = printReady
		? ((printReportResp as any)?.data as any)?.data
		: null;
	...
	const handleWithdraw = (e: React.FormEvent) => {
		...
		if (Number(withdrawAmount) > (savings?.general?.balance || 0)) {
			addToast({
				...
				message: "Nominal penarikan tidak boleh melebihi saldo Tabungan Umum.",
			});
			return;
		}
	...
	if (isStudentLoading || isSavingsLoading)
		return (
						...
						<div className="overflow-x-auto">
							<table className="min-w-full divide-y divide-gray-300">
								<thead className="bg-gray-50">
									<tr>
										<th className="py-2.5 pl-6 pr-3 text-left text-xs font-semibold text-gray-900 w-10">
											No.
										</th>
										<th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-900 w-24">
											Tanggal
										</th>
										<th className="px-3 py-2.5 text-left text-xs font-semibold text-gray-900">
											Keterangan
										</th>
										<th className="px-3 py-2.5 text-right text-xs font-semibold text-gray-900">
											Debit
										</th>
										<th className="px-3 py-2.5 text-right text-xs font-semibold text-gray-900">
											Kredit
										</th>
										<th className="px-3 py-2.5 text-right text-xs font-semibold text-gray-900 pr-6">
											Saldo
										</th>
									</tr>
								</thead>
								<tbody className="divide-y divide-gray-100 bg-white text-sm">
									...
									<tr className="bg-gray-50/50">
										...
										<td className="px-3 py-2" />
										<td
											className={`px-3 py-2 text-right tabular-nums font-semibold pr-6 ${printReport.saldo_awal < 0 ? "text-red-600" : ""}`}
										>
									...
									{printRows.map((row: TabunganSiswaRow, idx: number) => (
										<tr key={idx} className="hover:bg-gray-50">
											<td className="py-1.5 pl-6 pr-3 text-gray-500 tabular-nums">
												{idx + 1}.
											</td>
											...
											<td
												className={`px-3 py-1.5 text-right tabular-nums pr-6 ${row.saldo < 0 ? "text-red-600" : ""}`}
											>
									...
									{printRows.length > 0 && (
										<tr className="bg-gray-50 border-t-2 border-gray-300 font-semibold">
											...
											<td className="px-3 py-2.5 text-right tabular-nums">
												{formatCurrency(printReport.total_debit)}
											</td>
											<td className="px-3 py-2.5 text-right tabular-nums">
												{formatCurrency(printReport.total_credit)}
											</td>
									...
									<tr className="bg-gray-100 border-t font-bold">
										...
										<td className="px-3 py-2.5" />
										<td
											className={`px-3 py-2.5 text-right tabular-nums pr-6 ${printReport.saldo_akhir < 0 ? "text-red-600" : ""}`}
										>
											{formatCurrency(printReport.saldo_akhir)}
										</td>
									</tr>
								</tbody>
							</table>
								...
								<dl>
									...
									<dd>
										<div className="text-2xl font-bold text-gray-900">
											{formatCurrency(savings?.general?.balance || 0)}
										</div>
									</dd>
								</dl>
							</div>
						...
						<div className="flex items-center">
							...
							<div className="ml-5 w-0 flex-1">
								<dl>
									...
									<dd>
										<div className="text-2xl font-bold text-gray-900">
											{formatCurrency(savings?.mandatory?.balance || 0)}
										</div>
									</dd>
								</dl>
							</div>
				...
				>
					<div className="flex-1 overflow-y-auto px-4 py-6 sm:px-6 space-y-6">
						...
						<div className="text-sm text-gray-500 bg-gray-50 p-3 rounded border border-gray-200">
							<p>
								Penarikan ini hanya memotong <strong>Tabungan Umum</strong>.
								Tabungan Wajib (Berlian) tidak dapat ditarik secara manual di
								halaman ini.
							</p>
						</div>
					</div>
