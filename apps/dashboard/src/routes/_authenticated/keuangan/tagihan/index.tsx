...
function TagihanListPage() {
  ...
  const { data: invoicesData, isLoading } = useGetV1Invoices(
    {
      ...
      // The API might not natively support a free-text 'search' parameter based on swagger, 
      // but we add it if the backend supports it, otherwise it filters client-side (mocking it for now or passing anyway).
      ...(selectedType ? { type: selectedType } : {}),
      ...(selectedStatus ? { status: selectedStatus } : {}),
      ...(selectedClassGroup ? { class_group_id: Number(selectedClassGroup) } : {}),
  ...
  const getStatusBadge = (status: string, sisa: number) => {
    ...
    if (status === 'partial') return <Badge variant="warning">⚠ Sebagian (Sisa {formatCurrency(sisa)})</Badge>;
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm ring-1 ring-gray-900/5 space-y-4">
        <div className="flex flex-wrap gap-4 items-end">
          <div className="w-full sm:w-auto flex-1 min-w-[200px]">
            <label className="block text-sm font-medium leading-6 text-gray-900 mb-1">Pencarian (Nama/NIS)</label>
          ...
          <div className="w-full sm:w-auto">
            <label className="block text-sm font-medium leading-6 text-gray-900 mb-1">Rombel</label>
            <select
          ...
          <div className="w-full sm:w-auto">
            <label className="block text-sm font-medium leading-6 text-gray-900 mb-1">Jenis Tagihan</label>
            <select
              value={selectedType}
          ...
          <div className="w-full sm:w-auto">
            <label className="block text-sm font-medium leading-6 text-gray-900 mb-1">Status</label>
            <select
                  ...
                  return (
                    <tr key={invoice.id} className="hover:bg-gray-50 group">
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-900">
                        <Link to="/keuangan/tagihan/siswa/$id" params={{ id: invoice.student?.id.toString() }} className="font-semibold text-indigo-600 hover:text-indigo-900">
                          {invoice.student?.full_name}
                        </Link>
                        <div className="text-xs text-gray-500 mt-1">
                          {invoice.student?.active_enrollment?.class_group?.name || 'Tanpa Rombel'}
                        </div>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <div className="flex flex-col gap-1 items-start">
                          ...
                          <span className="text-xs text-gray-500">Total: {formatCurrency(Number(invoice.total_amount))}</span>
                        </div>
                      </td>
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <Link
                          to="/keuangan/tagihan/$id" params={{ id: invoice.id.toString() }}
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Menampilkan <span className="font-medium">{(page - 1) * 20 + 1}</span> sampai <span className="font-medium">{Math.min(page * 20, meta.total_items)}</span> dari <span className="font-medium">{meta.total_items}</span> tagihan
                </p>
              </div>
              <div>
