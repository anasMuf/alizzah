...
function PengaturanPenggunaComponent() {
	...
	const [userToDelete, setUserToDelete] = useState<DtoUserResponse | null>(
		null,
	);
	...
	const invalidate = () =>
		queryClient.invalidateQueries({ queryKey: getGetV1UsersQueryKey() });
	...
	return (
		<div className="space-y-6">
			...
			<ConfirmDialog
				...
				onConfirm={() =>
					userToDelete &&
					deleteMutation.mutate({ id: userToDelete.id as number })
				}
			...
			>
				<p>
					Anda yakin ingin menghapus <strong>{userToDelete?.full_name}</strong>?
		...
		>
			<form onSubmit={handleSubmit} className="space-y-6">
				...
				<FormField
					...
					type="password"
					placeholder={
						isEditing ? "Kosongkan jika tidak diubah" : "Minimal 8 karakter"
					}
					value={password}
