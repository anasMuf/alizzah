...
import type { DtoFeeConfigItemResponse, DtoCreateFeeConfigItemRequest } from '../../../../api/model';
import {
...
function PengaturanTarifIdComponent() {
  ...
  const { data: configResponse, isLoading: configLoading } = useGetV1FeeConfigsId(configId);
  const config = (configResponse?.data as any)?.data;

  const { data: itemsResponse, isLoading: itemsLoading } = useGetV1FeeConfigsIdItems(
    configId,
    filterCategory ? { category: filterCategory } : undefined,
  );
  const items: DtoFeeConfigItemResponse[] = (itemsResponse?.data as any)?.data || [];

  useEffect(() => {
    if (config) setSavingsRate(config.savings_admin_rate ?? 0);
  }, [config]);

  const invalidateConfig = () => queryClient.invalidateQueries({ queryKey: getGetV1FeeConfigsIdQueryKey(configId) });
  const invalidateItems = () => queryClient.invalidateQueries({ queryKey: getGetV1FeeConfigsIdItemsQueryKey(configId) });

  const updateConfigMutation = usePutV1FeeConfigsId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Rate admin tabungan berhasil diperbarui.' });
        invalidateConfig();
        setIsEditingRate(false);
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memperbarui konfigurasi.';
  ...
  const deleteItemMutation = useDeleteV1FeeConfigsIdItemsItemId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Item tarif berhasil dihapus.' });
        invalidateItems();
        setIsDeleteOpen(false);
        setItemToDelete(null);
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal menghapus item tarif.';
        addToast({ variant: 'error', title: 'Gagal', message: msg });
        setIsDeleteOpen(false);
      },
    },
  ...
  if (isLoading) {
    return <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 h-64 animate-pulse" />;
  }
  ...
  if (!config) {
    return (
      <EmptyState
        ...
        action={
          <Button variant="secondary" onClick={() => navigate({ to: '/pengaturan/tarif' })}>
            Kembali
          </Button>
        }
      />
  ...
  const groupedItems = items.reduce<Record<string, DtoFeeConfigItemResponse[]>>((acc, item) => {
    ...
    <div className="space-y-6">
      <div>
        ...
        <div className="sm:flex sm:items-center sm:justify-between">
          ...
          <div className="mt-4 sm:ml-4 sm:mt-0">
            <Button onClick={() => { setEditingItem(null); setIsItemFormOpen(true); }} className="flex items-center gap-2">
      ...
      <div className="bg-white rounded-xl shadow-sm ring-1 ring-gray-900/5 p-5">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-sm font-medium text-gray-500">Rate Admin Tabungan</h2>
            {isEditingRate ? (
              <div className="flex items-center gap-2 mt-1">
                ...
                <button
                  onClick={() => updateConfigMutation.mutate({ id: configId, data: { savings_admin_rate: savingsRate } })}
                  className="p-1 text-green-600 hover:text-green-700"
                >
                  <Save className="h-4 w-4" />
                </button>
                <button
                  onClick={() => { setSavingsRate(config.savings_admin_rate ?? 0); setIsEditingRate(false); }}
              ...
              <div className="flex items-center gap-2 mt-1">
                <p className="text-2xl font-bold text-gray-900">{config.savings_admin_rate ?? 0}%</p>
                <button onClick={() => setIsEditingRate(true)} className="p-1 text-gray-400 hover:text-indigo-600">
      ...
      {items.length === 0 ? (
        <EmptyState
          ...
          description={filterCategory ? 'Tidak ada item untuk kategori ini.' : 'Belum ada item tarif yang ditambahkan.'}
          action={
            !filterCategory ? (
              <Button onClick={() => { setEditingItem(null); setIsItemFormOpen(true); }} className="flex items-center gap-2">
                <Plus className="h-4 w-4" />
      ...
      <ConfirmDialog
        ...
        onConfirm={() =>
          itemToDelete && deleteItemMutation.mutate({ id: configId, itemId: itemToDelete.id as number })
        }
        ...
        cancelLabel="Batal"
      >
        <p>Anda yakin ingin menghapus <strong>{itemToDelete?.name}</strong>? Item yang sudah digunakan pada tagihan tidak dapat dihapus.</p>
      </ConfirmDialog>
    </div>
  );
}
          ...
          {items.map((item) => (
            <tr key={item.id} className="hover:bg-gray-50">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-right">
                {item.unit === 'percent' ? `${item.amount}%` : formatCurrency(item.amount ?? 0)}
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-right">
                <div className="flex justify-end gap-2">
                  <button onClick={() => onEdit(item)} className="p-1.5 text-gray-400 hover:text-indigo-600 rounded">
                    <Edit className="h-4 w-4" />
                  </button>
                  <button onClick={() => onDelete(item)} className="p-1.5 text-gray-400 hover:text-red-600 rounded">
...
function ItemFormSlideOver({
  ...
  const createMutation = usePostV1FeeConfigsIdItems({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Item tarif berhasil ditambahkan.' });
        onSuccess();
        onClose();
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal menambahkan item tarif.';
        addToast({ variant: 'error', title: 'Gagal', message: msg });
      },
    },
  });
  ...
  const updateMutation = usePutV1FeeConfigsIdItemsItemId({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Item tarif berhasil diperbarui.' });
        onSuccess();
        onClose();
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal memperbarui item tarif.';
        addToast({ variant: 'error', title: 'Gagal', message: msg });
      },
    },
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing && initialData) {
      updateMutation.mutate({ id: configId, itemId: initialData.id as number, data: formData });
    } else {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="category" className="block text-sm font-medium leading-6 text-gray-900 mb-2">Kategori</label>
            <select
              id="category"
              ...
              required
            >
              {Object.entries(DtoCreateFeeConfigItemRequestCategory).map(([, val]) => (
                <option key={val} value={val}>{CATEGORY_LABELS[val] || val}</option>
              ))}
            </select>
          </div>
          <div>
            <label htmlFor="unit" className="block text-sm font-medium leading-6 text-gray-900 mb-2">Satuan</label>
            <select
              id="unit"
              value={formData.unit}
              ...
              required
            >
              {Object.entries(DtoCreateFeeConfigItemRequestUnit).map(([, val]) => (
                <option key={val} value={val}>{UNIT_LABELS[val] || val}</option>
              ))}
            </select>
          </div>
        </div>
        ...
        <div>
          <label htmlFor="amount" className="block text-sm font-medium leading-6 text-gray-900 mb-2">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label htmlFor="level" className="block text-sm font-medium leading-6 text-gray-900 mb-2">Level</label>
            <select
              id="level"
              ...
              required
            >
              {Object.entries(DtoCreateFeeConfigItemRequestLevel).map(([, val]) => (
                <option key={val} value={val}>{LEVEL_LABELS[val] || val}</option>
              ))}
            </select>
          </div>
          <div>
            <label htmlFor="gender" className="block text-sm font-medium leading-6 text-gray-900 mb-2">Gender</label>
            <select
              id="gender"
              ...
              required
            >
              {Object.entries(DtoCreateFeeConfigItemRequestGender).map(([, val]) => (
                <option key={val} value={val}>{GENDER_LABELS[val] || val}</option>
              ))}
            </select>
          </div>
        </div>
