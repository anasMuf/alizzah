...
function PengaturanTarifComponent() {
  ...
  const feeConfigs: DtoFeeConfigResponse[] = (response?.data as any)?.data || [];
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="bg-red-50 p-4 rounded-md text-red-800">Gagal memuat data konfigurasi tarif.</div>
      ) : feeConfigs.length === 0 ? (
        <EmptyState
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {feeConfigs.map((fc) => (
            <button
              key={fc.id}
              onClick={() => navigate({ to: '/pengaturan/tarif/$id', params: { id: String(fc.id) } })}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <Badge variant="secondary">{fc.items?.length || 0} item tarif</Badge>
                  </div>
                </div>
      ...
      <CreateFeeConfigSlideOver
        ...
        existingYearIds={feeConfigs.map((fc) => fc.academic_year?.id).filter(Boolean) as number[]}
        onSuccess={() => queryClient.invalidateQueries({ queryKey: getGetV1FeeConfigsQueryKey() })}
      />
    </div>
  );
}
...
function CreateFeeConfigSlideOver({
  ...
  const availableYears = academicYears.filter((ay: any) => !existingYearIds.includes(ay.id));
  ...
  const createMutation = usePostV1FeeConfigs({
    mutation: {
      onSuccess: () => {
        addToast({ variant: 'success', title: 'Berhasil', message: 'Konfigurasi tarif berhasil dibuat.' });
        onSuccess();
        onClose();
      },
      onError: (error: Error) => {
        const msg = error instanceof ApiError ? error.message : 'Gagal membuat konfigurasi tarif.';
  ...
  return (
    <SlideOver
      ...
      footer={
        <>
          <Button variant="secondary" onClick={onClose} disabled={createMutation.isPending}>Batal</Button>
          <Button variant="primary" onClick={handleSubmit} disabled={createMutation.isPending || !academicYearId}>
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label htmlFor="academic_year_id" className="block text-sm font-medium leading-6 text-gray-900 mb-2">
            Tahun Ajaran
          </label>
          {availableYears.length === 0 ? (
            <p className="text-sm text-gray-500">Semua tahun ajaran sudah memiliki konfigurasi tarif.</p>
          ) : (
            <select
              id="academic_year_id"
        ...
        <div>
          <label htmlFor="savings_admin_rate" className="block text-sm font-medium leading-6 text-gray-900 mb-2">
            Rate Admin Tabungan (%)
          </label>
          <input
            ...
            required
          />
          <p className="mt-1 text-xs text-gray-500">Persentase biaya admin yang dikenakan pada tabungan siswa.</p>
        </div>
      </form>
    </SlideOver>
  );
