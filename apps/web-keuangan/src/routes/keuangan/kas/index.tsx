...
function KasPage() {
    ...
    const [activeTab, setActiveTab] = useState<'overview' | 'transaksi'>('overview');
    ...
    const { data: transactions, isLoading: isLoadingTransactions } = useKasTransaksi(token, { limit: 10 });

    return (
        <div className="p-8 max-w-[1400px] mx-auto space-y-8 pb-24 animate-in fade-in duration-700">
            {/* Modals */}
            <AnimatePresence>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div className="flex bg-slate-100 p-1.5 rounded-2xl w-fit shadow-sm border border-slate-200">
                    <button
                        ...
                        className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'overview' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                        <Wallet size={18} /> Overview Saldo
                    </button>
                    <button
                        onClick={() => setActiveTab('transaksi')}
                        className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'transaksi' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
                    >
                    ...
                    >
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="flex gap-2">
                                <button className="px-4 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50">Filter</button>
                                <button className="px-4 py-2 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50">Export</button>
                            </div>
                        </div>
                                    ...
                                    {transactions?.data?.map((trx: any) => (
                                        <tr key={trx.id} className="group hover:bg-slate-50 transition-all rounded-2xl overflow-hidden shadow-sm">
                                            ...
                                            <td className="px-4 py-4 bg-slate-50/50 group-hover:bg-slate-100/50 border-y border-slate-100 group-hover:border-slate-200 text-right">
                                                <p className={`font-black ${trx.tipeTransaksi === 'MASUK' ? 'text-emerald-600' :
                                                    trx.tipeTransaksi === 'KELUAR' ? 'text-rose-600' :
                                                        'text-slate-900'
                                                    }`}>
                                                    {trx.tipeTransaksi === 'KELUAR' ? '-' : ''}{formatCurrency(trx.nominal)}
                        ...
                        <div className="mt-8 flex justify-center py-4 border-t border-slate-100">
                            <div className="flex gap-2">
                                <button className="w-10 h-10 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400">1</button>
                            </div>
                        </div>
                    </motion.div>
                )}
